...
tax$tax.rate.adj <- tax$tax.rate/tax$CPIAUCSL
...
tax.by.inflation <- ggplot(tax) + aes(x = CPIAUCSL, y = tax.rate, label = year) + geom_text() +
...
adj.tax.by.inflation <- ggplot(melt(tax, c('year', 'budget', 'CPIAUCSL', 'assessed'))) +
  aes(x = CPIAUCSL, y = value, group = variable, label = year, linetype = variable) +
  labs(title = '') +
  scale_x_continuous('Value of a dollar in May 2012 dollars', labels = dollar) +
